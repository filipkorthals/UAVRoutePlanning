...
class HeuristicKorean:
    ...
    def create_grid(self, vertices_array: np.array, alpha: float = None) -> np.array:
        if alpha is None:
        ...
        grid_points = np.array([point for point in set_points if polygon.contains_point(tuple([point[0], point[1]]))])
    ...
    def traverse_the_grid(self, grid_points: np.array, starting_point: np.array, starting_direction: float) -> (np.array, np.array, np.array):
        ...
        return np.array(path), np.array(direction_history), np.array(turn_history)

    def calculate_path(self, img_path: str, starting_point: np.array, starting_direction: float) -> (np.array, np.array, np.array):
